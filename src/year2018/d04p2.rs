...
pub fn main() -> io::Result<()> {
    ...
    let (max_guard_id, most_consecutive): &(usize, usize) = &guard_sleep_times
        .iter()
        .map(|(&guard, &time_arr)| (guard, *time_arr.iter().max().unwrap()))
        .max_by_key(|&(_, consecutive)| consecutive)
        .unwrap();

    println!("{}", max_guard_id * most_consecutive);
