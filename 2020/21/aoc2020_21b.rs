...
fn main() -> Result<()> {
    ...
    let mut dangerous_ingr_aller: Vec<(&str, &str)> = known_has_allergen.iter().map(|(k, v)| (*k, *v)).collect();
    dangerous_ingr_aller.sort_by_key(|ingr_aller| ingr_aller.1);
    let dangerous_ingredients: Vec<&str> = dangerous_ingr_aller.into_iter().map(|(ingr, _)| ingr).collect();
    println!("Dangerous ingredients list:");
