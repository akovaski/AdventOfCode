...
fn main() -> Result<()> {
    ...
    let (mut seat_map, _) = create_seat_map(&lines);

    let mut changed = true;

    while changed {
        seat_map = simulate_round(&seat_map, &mut changed);
    }
...
fn simulate_round(seat_map: &HashMap<(i32, i32), bool>, changed: &mut bool) -> HashMap<(i32, i32), bool> {
    ...
    for (&seat, &taken) in seat_map {
        ...
        for idiff in -1..=1 {
            for jdiff in -1..=1 {
                ...
                if *seat_map.get(&(i + idiff, j + jdiff)).unwrap_or(&false) {
                    neighbors += 1;
                }
            }
        }
